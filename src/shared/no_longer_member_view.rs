...
live_design! {
    ...
    ICON_LEAVE = dep("crate://self/resources/icons/right_from_bracket.svg")
    ICON_FORBIDDEN = dep("crate://self/resources/icons/forbidden.svg")
    ICON_ENTER = dep("crate://self/resources/icons/right_to_bracket.svg")

    /// A view shown when the user is no longer a member of a room.
    pub NoLongerMemberView = {{NoLongerMemberView}} {
        ...
        content = <RoundedView> {
            ...
            buttons_view = <View> {
                ...
                rejoin_button = <RobrixIconButton> {
                    ...
                    icon_walk: {width: 16, height: 16, margin: {left: -2, right: 2} }
...
impl WidgetMatchEvent for NoLongerMemberView {
    fn handle_actions(&mut self, cx: &mut Cx, actions: &Actions, scope: &mut Scope) {
        let rejoin_button = self.button(ids!(rejoin_button));
...
impl NoLongerMemberView {
    ...
    pub fn show(&mut self, cx: &mut Cx, reason: NoLongerMemberReason) {
        ...
        // Update the title
        self.label(ids!(title)).set_text(cx, reason.title());

        // Update the description
        self.label(ids!(description)).set_text(cx, reason.description());

        // Show/hide the reason view if there's a specific reason
        if let Some(reason_text) = reason.reason_text() {
            self.view(ids!(reason_view)).set_visible(cx, true);
            self.label(ids!(reason_label)).set_text(cx, &format!("Reason: {}", reason_text));
        } else {
            self.view(ids!(reason_view)).set_visible(cx, false);
        }

        // Show/hide the rejoin button based on whether rejoin is possible
        self.button(ids!(rejoin_button)).set_visible(cx, reason.can_rejoin());

        // Update the icon based on the reason
        // Note: Icon update would require apply_over with svg_file
        // The default icon is set in the DSL definition
