...
use matrix_sdk::{
    ...
    ruma::{
        api::{
            Direction,
            client::{
                account::register::v3::Request as MatrixRegisterRequest,
        ...
        room::RoomType as MatrixRoomType,
        uint,
    },
...
use matrix_sdk_ui::{
    ...
    timeline::{
        LatestEventValue, RoomExt, TimelineEventItemId, TimelineFocus, TimelineItem,
        TimelineReadReceiptTracking, TimelineDetails,
    },
};
use robius_open::Uri;
use ruma::{
    ...
    api::client::{
        error::ErrorKind,
        uiaa::{AuthData, AuthType, Dummy, RegistrationToken, UiaaInfo},
    },
    events::tag::Tags,
};
use tokio::{
...
use crate::{
    ...
    home::{
        add_room::{CreateRoomResultAction, KnockResultAction},
...
) -> std::result::Result<(Client, ClientSessionPersisted), RegisterFlowError> {
...
pub enum MatrixRequest {
    ...
    /// Request to create a new private room or workspace on the current homeserver.
    CreateRoom {
        name: String,
        topic: Option<String>,
        is_space: bool,
                    ...
                    match res {
                        ...
                        Err(error) => {
                            ...
                            if direction == PaginationDirection::Backwards
                                && error_string.contains("M_INVALID_PARAM")
                                && error_string.contains("invalid batch token")
                            {
                                warning!(
                                    "Treating invalid pagination token as end-of-timeline for {timeline_kind}: {error_string}"
                                );
                                sender.send(TimelineUpdate::PaginationIdle {
            ...
            MatrixRequest::CreateRoom {
                ...
                let _create_room_task = Handle::current().spawn(async move {
                    ...
                    if is_space {
                        ...
                        creation_content.room_type = Some(MatrixRoomType::Space);
                        match Raw::new(&creation_content) {
                            Ok(raw_content) => request.creation_content = Some(raw_content),
                            Err(error) => {
                                Cx::post_action(CreateRoomResultAction::Failed {
                                    is_space,
                                    error: format!(
                                        "Failed to encode Matrix space creation content: {error}"
                                    ),
                                });
                                return;
                            }
                    ...
                    match client.create_room(request).await {
                        Ok(room) => {
                            ...
                            Cx::post_action(CreateRoomResultAction::Created {
                                room_name_id,
                                is_space,
                            });
                        }
                        Err(error) => {
                            error!(
                                ...
                                if is_space { "workspace" } else { "room" }
                            );
                            Cx::post_action(CreateRoomResultAction::Failed {
                                is_space,
...
async fn spawn_sso_server(
    ...
    if is_registration {
        ...
        Cx::post_action(RegisterAction::SsoRegistrationStatus {
            status:
                "Please wait while Matrix builds and configures the client object for registration."
                    .into(),
        });
    } else {
    ...
    Handle::current().spawn(async move {
        ...
        let Some((client, client_session)) = client_and_session else {
            ...
            post_failure(if let Some(err) = build_client_error {
                format!(
                    "Could not create client object. Please try to {action} again.\n\nError: {err}"
                )
            } else {
        ...
        post_status(
            ...
            if is_registration {
                "Please finish registration using your browser, and then come back to Robrix."
                    .into()
            } else {
