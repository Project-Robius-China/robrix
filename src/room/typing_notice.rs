...
live_design! {
    ...
    pub TypingNotice = {{TypingNotice}} {
        ...
        flow: Right
        align: {y: 0.5}
        padding: {left: 12.0, top: 5.0, bottom: 5.0, right: 10.0}
        show_bg: true,
        draw_bg: {
            color: #e8f4ff,
        }
        ...
        avatars_view = <View> {
            ...
            avatar1 = <Avatar> {
                visible: false
                width: (TYPING_AVATAR_SIZE)
                height: (TYPING_AVATAR_SIZE)
            }
            avatar2 = <Avatar> {
                visible: false
                width: (TYPING_AVATAR_SIZE)
        ...
        typing_label = <Label> {
            align: {x: 0.0, y: 0.5},
            padding: {left: 8.0, right: 0.0, top: 0.0, bottom: 0.0}
            draw_text: {
...
impl TypingNotice {
    ...
    fn show_or_hide(&mut self, cx: &mut Cx, typing_users: &[TypingUser]) {
        ...
        for (i, avatar_id) in avatar_ids.iter().enumerate() {
            let avatar_ref = self.view.avatar(*avatar_id);
