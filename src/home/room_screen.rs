        ...
        events::{
            ...
            room::{
                ...
                message::{
                    ...
                    NoticeMessageEventContent, ReplyWithinThread, RoomMessageEventContent,
...
use crate::{
    ...
    sliding_sync::{
        ...
        TimelineKind, TimelineRequestSender, UserPowerLevels, get_client, spawn_on_tokio,
...
pub struct RoomScreen {
    ...
    #[rust]
    bot_thinking_targets: Vec<TimelineEventItemId>,
}
    ...
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        if let Event::Actions(actions) = event {
            for (index, wr) in portal_list.items_with_actions(actions) {
                ...
                if let ReactionListAction::InterruptBotThinking { timeline_kind, .. } = actions
                    .find_widget_action(reaction_list.widget_uid())
                    .cast()
                {
                    ...
                    if interrupt_result.interrupted {
                        ...
                        match botfather::cancel_direct_stream_message(
                                ...
                                spawn_on_tokio(async move {
                                    match send_handle.abort().await {
                                        ...
                                        Ok(false) => {
                                                ...
                                                .await
                                            {
                                                enqueue_popup_notification(
                                                    format!(
                                                        "Failed to update interrupted bot stream message: {error}"
                                                    ),
                                                    PopupKind::Error,
                                                    Some(5.0),
                                                );
                                        ...
                                        Err(error) => enqueue_popup_notification(
                                            format!(
                                                "Failed to interrupt bot stream placeholder: {error}"
                                            ),
                                            PopupKind::Error,
                                            Some(5.0),
                                        ),
                            ...
                            botfather::DirectStreamHandleState::Missing => {
                                let replied_to = timeline_kind.thread_root_event_id().map(
            ...
            for action in actions {
                if let Some(BotfatherAction::StreamStarted {
                    room_id,
                    thread_root_event_id,
                    ..
                }) = action.downcast_ref()
                ...
                if let Some(BotfatherAction::StreamQueued { room_id, .. }) = action.downcast_ref() {
                    if self.room_id().is_some_and(|id| id.as_str() == room_id) {
                        self.add_bot_thinking_reaction();
                    }
                    continue;
            ...
            */
            ...
            self.jump_to_bottom_button(ids!(jump_to_bottom_button))
...
impl RoomScreen {
        ...
        while let Ok(update) = tl.update_receiver.try_recv() {
            ...
            match update {
                ...
                TimelineUpdate::NewItems {
                    ...
                    let own_user_id =
                        get_client().and_then(|client| client.user_id().map(ToOwned::to_owned));
                    ...
                    for item in tl.items.iter() {
                        if let Some(event_tl_item) = item.as_event() {
                            if let TimelineItemContent::MsgLike(msg_like_content) =
                                event_tl_item.content()
                            {
                                if let MsgLikeKind::Message(message) = &msg_like_content.kind {
                                    if let MessageType::Text(text_content) = message.msgtype() {
                                        ...
                                        {
                                            visible_bot_placeholders.push(
                                                bot_stream_header.thread_root_event_id.clone(),
                                            );
                                            ...
                                            if !is_live_placeholder && is_stale_placeholder {
                                                if let Some(send_handle) =
                                                    event_tl_item.local_echo_send_handle()
                                                {
                                                    spawn_on_tokio(async move {
                                                    ...
                                                    (own_user_id.as_ref(), event_tl_item.event_id())
                                                {
                                                    if event_tl_item.sender() == own_user_id
                                                        && tl
                                                            .pending_bot_placeholder_cleanup
                            ...
                            if let Some(event_id) = event_tl_item.event_id() {
                                    ...
                                    event_tl_item.content()
                                {
                                    if let MsgLikeKind::Message(message) = &msg_like_content.kind {
                                        if let MessageType::Text(text_content) = message.msgtype() {
                                            if let Some(sse_url) =
                                                parse_sse_header(&text_content.body)
                                            {
                                                // Get or create SSE stream state
                                                let sse_state = tl
                                                    .sse_streams
                                                    .entry(event_id.to_owned())
                                                    .or_insert_with(|| SseStreamState {
                                                        url: sse_url.clone(),
                ...
                TimelineUpdate::SseContentUpdate {
                    event_id,
                    content,
                    is_complete,
                } => {
                ...
                .cast_ref()
            {
                ...
                MessageAction::CancelSend(details) => {
                    ...
                    spawn_on_tokio(async move {
                        match send_handle.abort().await {
                            Ok(true) => {
                                let stopped_bot_request =
                                    local_created_at.is_some_and(|local_created_at| {
    ...
    fn clear_bot_thinking_reaction(&mut self) {
        ...
        let Some(queued_target) =
            (!self.bot_thinking_targets.is_empty()).then(|| self.bot_thinking_targets.remove(0))
        else {
            return;
        };
        let timeline_event_id =
            Self::resolve_bot_thinking_target_id(tl, &own_user_id, &queued_target)
                .or_else(|| {
                    Self::find_latest_own_message_with_bot_thinking_reaction_id(tl, &own_user_id)
                })
            ...
            TimelineEventItemId::TransactionId(target_txn_id) => {
                if let Some(item_id) = tl
                    ...
                    .find(|event| {
                        event
                            .transaction_id()
                            .is_some_and(|txn_id| txn_id == target_txn_id)
                    })
                    .map(Self::stable_event_identifier)
                {
                    return Some(item_id);
...
pub enum TimelineUpdate {
    ...
    SseContentUpdate {
        ...
        event_id: OwnedEventId,
        /// The new content to append to the message.
        content: String,
...
struct SseStreamState {
    ...
    url: String,
    /// Accumulated content from the SSE stream.
    accumulated_content: String,
    /// Whether the SSE stream is currently being fetched.
    is_fetching: bool,
...
fn populate_message_view(
                ...
                MessageType::Text(TextMessageEventContent {
                    ...
                    if existed && item_drawn_status.content_drawn {
                        ...
                        let (display_body, display_formatted) =
                            if let Some(bot_stream_header) = parse_bot_stream_header(body) {
                                if !botfather::is_live_direct_stream_placeholder(
                                    ...
                                    let display_body = match preview {
                                        Some(preview) if !preview.text.trim().is_empty() => {
                                            preview.text
                                        }
    ...
    let message_details = MessageDetails {
        ...
        timeline_event_id,
        item_id,
        related_event_id: msg_like_content
            .in_reply_to
            .as_ref()
    ...
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        if let Some(thread_root_event_id) = details.thread_root_event_id.as_ref() {
            ...
            let apply_hover = |cx: &mut Cx, bg_color: Vec4| {
                thread_root_summary.apply_over(
                    cx,
                    live! {
                        draw_bg: {
                            color: (bg_color)
...
///
/// The expected format is: `!SSE|<URL>|`
